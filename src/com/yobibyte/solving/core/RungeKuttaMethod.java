...
public class RungeKuttaMethod implements DiffEquSolver {
	...
	public void solve(Body body, List<Body> bodies) {
		...
		// * k1v.getY() * STEP / 2);
		//
		// varToCalc.setLocation(coords.getX() + k2p.getX() * STEP / 2,
		// coords.getY() + k2p.getY() * STEP / 2);
		...
		// k2v.getX() + 2 * k3v.getX() + k4v.getX()) * STEP / 6,
		// body.velocity.getY() + (k1v.getY() + 2 * k2v.getY() + 2 * k3v.getY() +
		// k4v.getY()) * STEP / 6);
		// body.coord.setLocation(body.coord.getX() + (k1p.getX() + 2 * k2p.getX() +
		// 2 * k3p.getX() + k4p.getX()) * STEP / 6, body.coord.getY() + (k1p.getY()
		// + 2 * k2p.getY() + 2 * k3p.getY() + k4p.getY()) * STEP / 6);

		double[] tmp = getF(bodies, body, body.coord);
		k1v.setLocation(tmp[0], tmp[1]);
		k1p = body.velocity;

		varToCalc.setLocation((coords.getX() + k1p.getX() * STEP / 2), coords.getY() + k1p.getY() * STEP / 2);
		tmp = getF(bodies, body, varToCalc);
		k2v.setLocation(tmp[0], tmp[1]);
		k2p.setLocation(body.velocity.getX() * k1v.getX() * STEP / 2, body.velocity.getY() * k1v.getY() * STEP / 2);

		varToCalc.setLocation((coords.getX() + k2p.getX() * STEP / 2), coords.getY() + k2p.getY() * STEP / 2);
		tmp = getF(bodies, body, varToCalc);
		k3v.setLocation(tmp[0], tmp[1]);
		k3p.setLocation(body.velocity.getX() * k2v.getX() * STEP / 2, body.velocity.getY() * k2v.getY() * STEP / 2);

		varToCalc.setLocation((coords.getX() + k3p.getX() * STEP / 2), coords.getY() + k3p.getY() * STEP / 2);
		tmp = getF(bodies, body, varToCalc);
		k4v.setLocation(tmp[0], tmp[1]);
		k4p.setLocation(body.velocity.getX() * k3v.getX() * STEP / 2, body.velocity.getY() * k3v.getY() * STEP / 2);
		...
		// fy += f * deltaY / deltaR;
		// }
		// body.velocity.setLocation(body.velocity.getX() + STEP * fx / body.mass,
		// body.velocity.getY() + STEP * fy / body.mass);
		// body.coord.setLocation(body.coord.getX() + body.velocity.getX() * STEP,
		// body.coord.getY() + body.velocity.getY() * STEP);
		// }

	}

	public double[] getF(List<Body> bodies, Body body, Point2D varToCalc) {
		Iterator<Body> it = bodies.iterator();
		...
		if (it.hasNext()) {
			...
			if (currBody != body) {
				...
				double deltaR = deltaX * deltaX + deltaY * deltaY;
				f = Constants.GRAVITY * body.mass * currBody.mass / deltaR;
				deltaR = Math.sqrt(deltaR);
				fx += f * deltaX / deltaR;
				fy += f * deltaY / deltaR;
			}
